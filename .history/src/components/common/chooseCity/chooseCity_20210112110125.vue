<template>
    <div class="chooseCity">
      <Select  v-model="model1" style="min-width:10px;max-width: 160px;" @on-change="deleteModel(1)">
        <Option class="choose"  v-for="item in firstFloorDatas" :value="item.id" :key="item.id">{{ item.name }}</Option>
      </Select>
      <Select v-model="model2" style="min-width:10px;max-width: 160px;" v-if="showSecond" @on-change="deleteModel(2)">
        <Option v-for="item in secondFloorDatas" :value="item.id" :key="item.id">{{ item.name }}</Option>
      </Select>
      <Select v-model="model3" style="min-width:10px;max-width: 160px;" v-if="showThird" >
