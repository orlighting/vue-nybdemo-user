  ...
  <div>
    ...
    <div class="card1">
      <div class="chartUser">
        ...
        <el-form :model="declareForm" status-icon label-width="30px" class="demo-ruleForm" style="padding-right: 30px" enctype='multipart/form-data' :rules="rules">
          <div class="helpinfo">
...
export default {
  ...
  data() {
      var teleNum = (rule, value, callback) => {
        if (value === '') {
          callback(new Error('请输入密码'));
        } else {
            var pwdRegex = new RegExp('(^[0-9]{3,4}\-[0-9]{3,8}$)|(^[0-9]{3,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}13[0-9]{9}$)');
            if (!pwdRegex.test(value)) 
            {
                callback(new Error("您输入的号码有误，请重新输入"));
            }
          if (this.declareForm.teleNum !== '') {
            this.$refs.declareForm.teleNum('checkTeleNum');
          }
          callback();
        }
      };
    return {
      ...
      rules:{
          checkTeleNum: [
            { validator: teleNum, trigger: 'blur' }
          ],
      }
    };
  },
